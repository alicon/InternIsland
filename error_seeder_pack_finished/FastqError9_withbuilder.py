...
def parse_cmdline_params(arg_list=None):
    ...
    parser.add_argument("--count", "-w", choices=[0, 1], default=0,
                        help="Not required. "
                             "Pick 0 for no error counting."
                             "Pick 1 for the amount of errors",
                        required=False,
                        type=int)
...
def main(args):
    ...
    if errors == 1:
        dict_and_count = decide_if_error_1(fastq_dict, infile, status_updates, pileup_dict)
        error_dict = dict_and_count[0]
        error_count = dict_and_count[1]
    elif errors == 0:
        dict_and_count= decide_if_error_0(fastq_dict)
        error_dict = dict_and_count[0]
        error_count = dict_and_count[1]
    if status_updates:
